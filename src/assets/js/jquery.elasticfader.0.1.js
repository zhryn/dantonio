(function($) {
	...
	$.fn.elasticFader = function( options ) {
		...
		return this.each(function() {
			...
			var myH = $(window).height() - settings.sliderOffsetY;
			$(sliderContainer).css("height", (myH + "px"));
			var i = 0;
			...
			function fitImage(container, img, imgW, imgH) {
				...
				//console.info(sc + "   // " + container.attr("class") + "  // " + Math.random());
				...
				if (settings.centerImage) {
					...
					if (container.height() < img.height()) {
						var imgOffsetY = (img.height() - container.height()) / 2;
			...
			function openSlide(slideNumber) {
				...
				$("#" + sliderContainer.attr("id") + " li").each(function() {
					...
					var opacity = 0;
					// var delay = settings.animationSpeed;
					var delay = 0;
					var speed = settings.animationSpeed / 2;
					if (slideNumber == i){
						opacity = 1;
						delay = 0;
			...
			if(settings.navDots) {
				...
				$(settings.navContainer + " li ").click(function() {
					...
					var clickedSlideIndex = parseInt($(this).attr('class'));
